...
/** A negative bar_range shortens the bar when the neuron fires. */
static void test_actuator_negative_range_shortens_bar()
{
    Robot r = make_base_robot();
    r.addNeuron(Neuron(0.5));
    r.addActuator(Actuator(0, 0, -0.005));
...
/** bar_range > 0.01 m is clamped to ≤ 0.01 m per cycle. */
...
/** bar_range < -0.01 m is clamped to ≥ -0.01 m per cycle. */
static void test_actuator_clamp_negative()
{
    Robot r = make_base_robot();
    r.addNeuron(Neuron(0.5));
    r.addActuator(Actuator(0, 0, -0.05));  // -5 cm — exceeds the limit
...
static void test_combined_tick_and_apply()
{
    ...
    r.addActuator(Actuator(1, 1, -0.005));   // neuron 1 → bar 1 shortens
    ...
    // Neuron 1 fired → bar 1 shortens by 0.005
    ...
    // Neuron 1 was silent → bar 1 unchanged from previous cycle
    CHECK_NEAR(sim.rest_lengths_[1], L0_bar1 - 0.005, EPS);
}
...
int main()
{
    ...
    run("actuator: negative range shortens bar",  test_actuator_negative_range_shortens_bar);
    run("actuator: positive clamp at 1 cm",       test_actuator_clamp_positive);
    run("actuator: negative clamp at -1 cm",      test_actuator_clamp_negative);
